...
function handleKeyDown(e) {
    try {
        ...
        if (isTouchActiveGame && isInGameState) {
            ...
            // Enter wordt hier niet meer gebruikt om terug te gaan naar het menu.
            return;
        }
            ...
            if (!isPaused) {
                if (!isManualControl) { // AI Demo
                    if (isPlayerTwoAI && selectedGameMode === 'normal' && currentPlayer === 2) {
                        ...
                        if (e.key === "Escape") { // Alleen Escape stopt AI demo
                            ...
                            // Elke andere toets (NIET Enter, NIET p/P, NIET modifiers) toont menu voor 1P AI Demo
                            if(typeof showMenuState === 'function') showMenuState();
                        }
                        // Enter doet hier niets meer voor AI demo exit
                    }
                } else { // isManualControl
                    switch (e.code) {
                        ...
                        case "Escape": // Escape stopt het spel en gaat naar menu
                            if(typeof stopGameAndShowMenu === 'function') stopGameAndShowMenu();
                            break;
                        case "Enter": // Enter is nu voor schieten
                            keyboardP1ShootDown = true;
                            if (isTwoPlayerMode && !isPlayerTwoAI) { // Als 2P mode en P2 is geen AI
                                keyboardP2ShootDown = true;
                            }
                            break;
                    }
                    // Fallbacks voor lowercase letters (P2)
    ...
    try {
        ...
        switch (e.code) {
            ...
            case "KeyI": case "Numpad0":
                ...
                break;
            case "Enter": // Nieuw: Enter toets loslaten
                keyboardP1ShootDown = false;
                if (selectedFiringMode === 'single') p1JustFiredSingle = false;
